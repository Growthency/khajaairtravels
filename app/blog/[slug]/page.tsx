...
export default function BlogPost({ params }: Props) {
  ...
  if (!post) notFound();
  const related = recentPosts(3, post.slug);
      ...
      <article>
        <header className="relative isolate overflow-hidden bg-ink text-white pt-28 pb-20 md:pt-36 md:pb-24">
          ...
          <Container>
            <Link
              href="/blog"
              className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.22em] text-emerald-300 hover:text-emerald-200"
            >
            ...
            <div className="mt-6 max-w-3xl">
              ...
              <h1 className="mt-5 font-display text-3xl font-bold leading-[1.08] tracking-tight md:text-[48px]">
                {post.title}
              </h1>
              ...
              <div className="lg:col-span-8">
                ...
                <div className="mt-12 flex flex-wrap gap-2">
                  {post.tags.map((t) => (
                    <span
                      key={t}
                      className="rounded-full bg-paper-2 px-3 py-1 text-xs font-medium text-ink-soft ring-1 ring-inset ring-border"
                    >
                      #{t}
                    </span>
                  ))}
                        ...
                        <li key={r.slug}>
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-1 line-clamp-2 text-sm font-semibold text-ink group-hover:text-sky-700">
                                {r.title}
                              </div>
                            </div>
                          </Link>
        ...
        <section className="border-t border-border bg-paper-2 py-16">
          <Container>
            <div className="mb-10 flex items-end justify-between">
              ...
              <Link
                href="/blog"
                className="hidden items-center gap-1.5 text-sm font-semibold text-sky-700 hover:text-sky-800 md:inline-flex"
              >
                All articles
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  href={`/blog/${r.slug}`}
                  className="group flex flex-col overflow-hidden rounded-3xl border border-border bg-white transition-all hover:-translate-y-1 hover:shadow-card-hover"
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <h3 className="mt-2 line-clamp-2 font-display text-lg font-bold text-ink group-hover:text-sky-700">
                      {r.title}
                    </h3>
